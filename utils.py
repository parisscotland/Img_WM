...
class WMKey:
    def __init__(self, src_img, _S = None):
        if _S is None:
    ...
    def innerProduct(self, another_key):
        return np.sum(self.S*another_key.S)
    ...
    def visualize_S(self):
        ...
        for i in range(3):
            # Map 1 to white and -1 to black for each channel
            channel_matrix = np.where(self.S[:, :, i] == 1, 255, 0)
            axs[i].imshow(channel_matrix, cmap='gray', interpolation='nearest')
...
class WaterMarker:
    ...
    @staticmethod
    def extractWM(tgt_img, wm_key, fpr = 0.0005):
